...
def main():
    ...
    # Create dummy dataloader just to initialize ValidationRunner properly
    # so we can use its built-in T5 encoder
    class DummyLoader:
        def __iter__(self):
            return iter([])
    ...
    sampler = ValidationSampler(
        model,
        None, # VAE not needed for pixel space
        device=device,
        num_steps=config.sampling.num_steps,
        text_scale=3.0,
        dino_scale=0.0, # Zero DINO influence
        self_guidance=False, # Standard dual CFG
        prediction_type=getattr(config.model, 'prediction_type', 'v_prediction'),
    )

    out_dir = exp_dir / "novel_prompts"
    out_dir.mkdir(exist_ok=True)

    prompts = [
        "A photo of a person with bright green hair and a cybernetic mechanical eye.",
        "An old man with a long white beard wearing a futuristic neon space helmet.",
        "A highly detailed portrait of a woman made entirely of cracked porcelain.",
        "A child with blue skin eating a glowing yellow apple.",
        "A close-up portrait of a fantasy elven warrior with purple war paint on their face."
    ]

    print("Generating novel prompts to test generalization...")
    for i, prompt in enumerate(prompts):
        ...
        # Dummy DINO embeddings (since dino_scale=0, values don't matter, just shape)
        dino_emb = torch.zeros(1, 1024, device=device)
        # Sequence length 256 for patches
        ...
        with torch.no_grad():
            gen_image = sampler.generate(
                ...
                self_guidance=False,
                dino_scale=0.0,
                text_scale=4.0 # Push text guidance a bit to force adherence
            )[0] # (3, H, W)
        
        # gen_image is in [-1, 1], map to [0, 1]
        gen_image = (gen_image + 1.0) / 2.0
        gen_image = gen_image.clamp(0, 1)
        save_path = out_dir / f"novel_{i:02d}.png"
        save_image(gen_image, save_path)
        
        with open(out_dir / f"novel_{i:02d}.txt", "w") as f:
            f.write(prompt)
