...
such as Photoroom's PRX series have shown that pixel-space prediction combined
with token routing, representation alignment, and second-order-flavored
optimizers can compress the cost dramatically. Yet most published recipes are
...
pixel-space Diffusion Transformer (NanoDiT) trained on a curated vertical
dataset (FFHQ portraits). We run a sequence of controlled arms, each isolating
...
clean image $x_0$ directly (x-prediction) under a rectified-flow objective, with
no VAE. Conditioning is multi-modal (``quad-conditioning''): T5-Large text
embeddings, DINOv3 CLS (global style) and patch (spatial layout) features, and
DWPose whole-body keypoints. All conditioning signals are pre-computed and
...
\paragraph{Metrics.} We report LPIPS (lower is better) at the final step-5000
...
converge. We note the known \emph{capacity-mismatch trap}: DINOv3 features lack
...
projection (rank 8) nearly doubles throughput ($0.0145\to0.0243$ it/s) and
...
may shift at larger scale. The FP8 quality gap, while small, is real and
measured at one budget only.
